...
//	private CountryOriented countries = new CountryOriented();
	...
	/**
	 * Add to the model the Resources that contain the Countries as retrieved
	 * from the respective dictionary
	 * 
	 * @param Model
	 *            the model we are currently working with
	 * @param DictionaryItems
	 *            the countries as retrieved from the respective dictionary
	 */
//	public void addCountriesToModel(Model model, DictionaryItems countryList) {
//
//		for (DictionaryItem item : countryList.getItems()) {
//			Resource fekTypeResource = model.createResource(Ontology.instancePrefix + "Country/" + item.getUid(),
//					Ontology.countryResource);
...
//	}
	...
	/**
	 * Add to the model the Resources that contain the Currencies as retrieved
	 * from the respective dictionary
	 * 
	 * @param Model
	 *            the model we are currently working with
	 * @param DictionaryItems
	 *            the currencies as retrieved from the respective dictionary
	 */
...
//				Resource fekTypeResource = model.createResource(Ontology.instancePrefix + "Currency/" + item.getUid(),
//						Ontology.currencyResource);
...
//	}
	...
	public void addAllOrganizationsToModel(Model model, Organizations organizationsList) {
		 ...
		 * newOrg.setAbbreviation("sintomografiaAAAA"); newOrg.setLabel(
		 * "LABEL malakiaAAAA"); newOrg.setUid("23185"); newOrg.setLatinName(
		 * "latin name"); newOrg.setCategory("cccCCC");
		 * newOrg.setStatus("statusSSS"); newOrg.setFekYear("10");
		 * newOrg.setFekNumber("10"); newOrg.setFekIssue("fektype_A");
		...
		for (Organization org : organizationsList.getOrganizations()) {
			...
			if (org.getSupervisorId() != null) {
				supervisor = handleRequests.searchOrganization(org.getSupervisorId()); // find
																						// the
																						// details
																						// of
																						// the
			...
			try {
				...
				if (supervisor != null) {
					out.println(org.getVatNumber() + ";" + hm.cleanInputData(org.getLabel()) + ";" + org.getUid() + ";"
							+ org.getSupervisorId() + ";" + orgUri);
				} else {
					out.println(org.getVatNumber() + ";" + hm.cleanInputData(org.getLabel()) + ";" + org.getUid() + ";"
							+ ";" + orgUri);
				}
			...
			if (model.containsResource(orgToSearch)) { // edit existing resource
				...
				if (org.getVatNumber() != "") {
					if (orgToSearch.getProperty(Ontology.vatId) != null) {
						if (!orgToSearch.getProperty(Ontology.vatId).getObject().toString()
								.equals(org.getVatNumber() + "^^http://www.w3.org/2001/XMLSchema#string")) {
				...
				if (legalEntity != null) {
					...
					orgToSearch.removeAll(Ontology.validVatId);
					orgToSearch.addLiteral(Ontology.validVatId,
				...
				if ((org.getWebsite() != null) && (org.getWebsite() != "")) {
					if (orgToSearch.getProperty(Ontology.organizationWebsite) != null) {
						if (!orgToSearch.getProperty(Ontology.organizationWebsite).getObject().toString()
								.equals(org.getWebsite() + "^^http://www.w3.org/2001/XMLSchema#string")) {
				...
				if ((org.getOdeManagerEmail() != null) && (org.getOdeManagerEmail() != "")) {
					if (orgToSearch.getProperty(Ontology.odeManagerEmail) != null) {
						if (!orgToSearch.getProperty(Ontology.odeManagerEmail).getObject().toString()
								.equals(org.getOdeManagerEmail() + "^^http://www.w3.org/2001/XMLSchema#string")) {
				...
				if (orgToSearch.getProperty(RDFS.label) != null) {
					if (!orgToSearch.getProperty(RDFS.label).getObject().toString()
							.equals(org.getLatinName() + "@en")) {
				...
				if (orgToSearch.getProperty(Ontology.legalName) != null) {
					if (!orgToSearch.getProperty(Ontology.legalName).getObject().toString()
							.equals(hm.cleanInputData(org.getLabel()) + "@el")) {
						orgToSearch.removeAll(Ontology.legalName);
						orgToSearch.addProperty(Ontology.legalName,
								model.createLiteral(hm.cleanInputData(org.getLabel()), "el"));
					}
				} else {
					orgToSearch.addProperty(Ontology.legalName,
							model.createLiteral(hm.cleanInputData(org.getLabel()), "el"));
				...
				if (orgToSearch.getProperty(Ontology.orgCategory) != null) {
					if (!orgToSearch.getProperty(Ontology.orgCategory).getObject().toString()
							.equals(Ontology.instancePrefix + "OrganizationCategory/" + org.getCategory())) {
						orgToSearch.removeAll(Ontology.orgCategory);
						orgToSearch.addProperty(Ontology.orgCategory, model
								.getResource(Ontology.instancePrefix + "OrganizationCategory/" + org.getCategory()));
					}
				} else {
					orgToSearch.addProperty(Ontology.orgCategory,
							model.getResource(Ontology.instancePrefix + "OrganizationCategory/" + org.getCategory()));
				}
				...
				if (orgToSearch.getProperty(Ontology.orgStatus) != null) {
					if (!orgToSearch.getProperty(Ontology.orgStatus).getObject().toString()
							.equals(statusIndividualUri[0])) {
				...
				if (org.getOrganizationDomains() != null) {
					...
					for (String orgDomain : org.getOrganizationDomains()) {
						orgToSearch.addProperty(Ontology.orgActivity,
								model.getResource(Ontology.instancePrefix + "OrganizationDomain/" + orgDomain));
					}
				} else {
					orgToSearch.removeAll(Ontology.orgActivity);
				}
				...
				if ((org.getAbbreviation() != null) && (org.getAbbreviation() != "")) {
					if (orgToSearch.getProperty(Ontology.organizationAbbreviation) != null) {
						if (!orgToSearch.getProperty(Ontology.organizationAbbreviation).getObject().toString()
								.equals(hm.cleanInputData(org.getAbbreviation()) + "@el")) {
							orgToSearch.removeAll(Ontology.organizationAbbreviation);
							orgToSearch.addProperty(Ontology.organizationAbbreviation,
									model.createLiteral(hm.cleanInputData(org.getAbbreviation()), "el"));
						}
					} else {
						orgToSearch.addProperty(Ontology.organizationAbbreviation,
								model.createLiteral(hm.cleanInputData(org.getAbbreviation()), "el"));
				...
				if (org.getSupervisorId() != null) {
					if (orgToSearch.getProperty(Ontology.hasSupervisorOrganization) != null) {
						if (supervisor != null) {
							if (!orgToSearch.getProperty(Ontology.hasSupervisorOrganization).getObject().toString()
									.equals("http://linkedeconomy.org/resource/Organization/"
											+ supervisor.getVatNumber())) {
				...
				Resource orgResource = model.createResource(Ontology.instancePrefix + "Organization/" + orgUri,
						Ontology.organizationResource);
				model.createResource(Ontology.instancePrefix + "Organization/" + orgUri,
				...
				orgResource.addProperty(Ontology.legalName,
						model.createLiteral(hm.cleanInputData(org.getLabel()), "el"));
				orgResource.addProperty(Ontology.orgCategory,
						model.getResource(Ontology.instancePrefix + "OrganizationCategory/" + org.getCategory()));
				...
				if (legalEntity != null) {
					...
					orgResource.addLiteral(Ontology.validVatId,
							model.createTypedLiteral(false, XSDDatatype.XSDboolean));
				}
				...
				if (org.getOrganizationDomains() != null) {
					for (String orgDomain : org.getOrganizationDomains()) {
						orgResource.addProperty(Ontology.orgActivity,
								model.getResource(Ontology.instancePrefix + "OrganizationDomain/" + orgDomain));
					}
				}

				if ((org.getAbbreviation() != null) && (org.getAbbreviation() != "")) {
					orgResource.addProperty(Ontology.organizationAbbreviation,
							model.createLiteral(hm.cleanInputData(org.getAbbreviation()), "el"));
				}
	...
	public void addSignerToModel(Model model, Resource orgResource, Signers signersList) {
		...
		for (Signer signer : signersList.getSigners()) {
			Resource signerResource = model.createResource(Ontology.instancePrefix + "Signer/" + signer.getUid(),
					Ontology.personResource);
			signerResource.addLiteral(Ontology.signerId, signer.getUid());
			signerResource.addLiteral(Ontology.signerActive, signer.isActive());
			// signerResource.addProperty(Ontology.lastName,
			// model.createLiteral(signer.getLastName(), "el"));
			// signerResource.addProperty(Ontology.firstName,
			// model.createLiteral(signer.getFirstName(), "el"));
			...
			// signerResource.addProperty(Ontology.belongsTo,
			// model.getResource(Ontology.instancePrefix + "Organization/" +
			// signer.getOrganizationId()));
			...
			for (SignerUnit signUnit : signer.getUnits()) {
				...
				if (signUnit.getPositionId() != null) {
					membershipResource = model.createResource(
							Ontology.instancePrefix + "Membership/" + signer.getUid() + "_position",
							Ontology.membershipResource);
					membershipResource.addProperty(Ontology.member,
							model.getResource(Ontology.instancePrefix + "Signer/" + signer.getUid()));
					membershipResource.addProperty(Ontology.role,
							model.getResource(Ontology.instancePrefix + "Role/" + signUnit.getPositionId()));
					orgObject = handleRequests.searchOrganization(signer.getOrganizationId()); // find
																								// the
																								// details
																								// of
																								// the
																								// Organization
					if (orgObject != null) {
						membershipResource.addProperty(Ontology.organization, model
								.getResource(Ontology.instancePrefix + "Organization/" + orgObject.getVatNumber()));
					}
				...
				membershipResource = model.createResource(
						Ontology.instancePrefix + "Membership/" + signer.getUid() + "_sign",
						Ontology.membershipResource);
				membershipResource.addProperty(Ontology.member,
						model.getResource(Ontology.instancePrefix + "Signer/" + signer.getUid()));
				membershipResource.addProperty(Ontology.role,
						model.getResource(Ontology.instancePrefix + "Role/Signer"));
				...
				if (signer.isHasOrganizationSignRights()) {
					if (signUnit.getUid() != null) {
						orgObject = handleRequests.searchOrganization(signUnit.getUid()); // find
																							// the
																							// details
																							// of
																							// the
					...
					if (signUnit.getUid() != null) {
						membershipResource.addProperty(Ontology.organization,
								model.getResource(Ontology.instancePrefix + "OrganizationalUnit/" + signUnit.getUid()));
					}
				}
	...
	/**
	 * Add to the model we are currently working with the Classes and their
	 * individuals from the the dictionaries: ORG_CATEGORY, ORG_UNIT_CATEGORY,
	 * ORG_DOMAIN, THK and VAT_TYPE.
	 * 
	 * @param Model
	 *            the model we are currently working with
	 * @param DictionaryItems
	...
	public void addDiavgeiaRelatedConceptsToModel(Model model, DictionaryItems dictItemsList) {
		...
		if (dictItemsList.getName().equalsIgnoreCase("ORG_CATEGORY")) {
			for (DictionaryItem item : dictItemsList.getItems()) {
				/** itemResource **/
				Resource itemResource = model.createResource(
						Ontology.instancePrefix + "OrganizationCategory/" + item.getUid(),
						Ontology.organizationCategoryResource);
				model.createResource(Ontology.instancePrefix + "OrganizationCategory/" + item.getUid(),
			...
			// /** itemResource **/
			// Resource deyaResource =
			// model.createResource(Ontology.instancePrefix +
			// "OrganizationCategory/" + "DEYA",
			// Ontology.organizationCategoryResource);
			// model.createResource(Ontology.instancePrefix +
			// "OrganizationCategory/" + "DEYA", Ontology.conceptResource);
			// /** configure prefLabels **/
			// String[] orgCatNames = hm.findOrganizationCategoryName("DEYA");
			// deyaResource.addProperty(Ontology.prefLabel,
			// model.createLiteral(orgCatNames[0], "el"));
			// deyaResource.addProperty(Ontology.prefLabel,
			// model.createLiteral(orgCatNames[1], "en"));
		} else if (dictItemsList.getName().equalsIgnoreCase("ORG_UNIT_CATEGORY")) {
			for (DictionaryItem item : dictItemsList.getItems()) {
				/** itemResource **/
				Resource itemResource = model.createResource(
						Ontology.instancePrefix + "OrganizationalUnitCategory/" + item.getUid(),
						Ontology.orgUnitCategoryResource);
				model.createResource(Ontology.instancePrefix + "OrganizationalUnitCategory/" + item.getUid(),
						Ontology.conceptResource);
				/** configure prefLabels **/
			...
			for (DictionaryItem item : dictItemsList.getItems()) {
				/** itemResource **/
				Resource itemResource = model.createResource(
						Ontology.instancePrefix + "OrganizationDomain/" + item.getUid(),
						Ontology.organizationDomainResource);
				model.createResource(Ontology.instancePrefix + "OrganizationDomain/" + item.getUid(),
						Ontology.conceptResource);
				/** configure OrganizationDomain broader **/
				if ((item.getParent() != null) && (item.getParent() != "")) {
					itemResource.addProperty(Ontology.broader,
							model.createResource(Ontology.instancePrefix + "OrganizationDomain/" + item.getParent(),
									Ontology.conceptResource));
				}
				/** configure prefLabels **/
			...
			for (DictionaryItem item : dictItemsList.getItems()) {
				/** itemResource **/
				Resource itemResource = model.createResource(
						Ontology.instancePrefix + "ThematicCategory/" + item.getUid(),
						Ontology.thematicCategoryResource);
				model.createResource(Ontology.instancePrefix + "ThematicCategory/" + item.getUid(),
						Ontology.conceptResource);
				/** configure prefLabel **/
			...
			for (DictionaryItem item : dictItemsList.getItems()) {
				/** itemResource **/
				Resource itemResource = model.createResource(Ontology.instancePrefix + "VatType/" + item.getUid(),
						Ontology.vatTypeResource);
	 ...
	 *            the list containing the positions
	 */
	public void addAllPositionsToModel(Model model, ArrayList<Position> positionsList) {
		for (Position position : positionsList) {
			/** itemResource **/
			Resource positionResource = model.createResource(Ontology.instancePrefix + "Role/" + position.getUid(),
					Ontology.roleResource);
		...
		/* add signer Role */
		/** signerResource **/
		Resource positionResource = model.createResource(Ontology.instancePrefix + "Role/Signer",
				Ontology.roleResource);
	...
	/**
	 * Add to the model the Position Types.
	 * 
	 * @param Model
	 *            the model we are currently working with
	 */
	public void addPositionTypeToModel(Model model) {

		List<String> positionList = Arrays.asList("Μονομελές Όργανο", "ΕΙΔΙΚΟΣ ΓΡΑΜΜΑΤΕΑΣ Σ.Δ.Ο.Ε.",
				"Γενικός Γραμματέας Υπουργείου");

		for (String position : positionList) {
			String[] positionDtls = hm.findPositionTypeIndividual(position);
			/** positionResource **/
			Resource positionResource = model.createResource(positionDtls[0], Ontology.positionResource);
			model.createResource(positionDtls[0], Ontology.conceptResource);
			/** configure prefLabel **/
			positionResource.addProperty(Ontology.prefLabel, positionDtls[1], "el");
			positionResource.addProperty(Ontology.prefLabel, positionDtls[2], "en");
		}
	...
	/**
	 * Add to the model the Budget Kinds.
	 * 
	 * @param Model
	 *            the model we are currently working with
	 */
	public void addBudgetKindToModel(Model model) {

		List<String> kindList = Arrays.asList("Φορέα", "Κρατικός");

		for (String kind : kindList) {
			String[] kindDtls = hm.findBudgetKindIndividual(kind);
			/** BudgetKindResource **/
			Resource budgetKindResource = model.createResource(kindDtls[0], Ontology.budgetKindResource);
			model.createResource(kindDtls[0], Ontology.conceptResource);
			/** configure prefLabel **/
			budgetKindResource.addProperty(Ontology.prefLabel, kindDtls[1], "el");
			budgetKindResource.addProperty(Ontology.prefLabel, kindDtls[2], "en");
		}
