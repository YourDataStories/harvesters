...
public class Main {
	...
	private static void actions() throws IOException {
		...
		if (Configuration.searchMontlhy) {
			...
			List<String> dictionaries = Arrays.asList("ORG_CATEGORY", "ORG_UNIT_CATEGORY", "ORG_DOMAIN", "THK", "VAT_TYPE");
			
			for (String dict : dictionaries) {
				dictItemsList = handleRequests.getDictionaryItems(dict); //Fetch the items of the provided dictionary 
				monthlyRdfActions.addDiavgeiaRelatedConceptsToModel(model, dictItemsList); //create their SKOS Concepts
			}
			...
			Organizations organizationsList = handleRequests.getAllOrganizations();
			monthlyRdfActions.addAllOrganizationsToModel(model, organizationsList);
		...
		if (Configuration.searchPeriod) {
			...
			LocalDate endDate = new LocalDate("2015-12-16"); //this date is not retrieved
	...
	private static void dailyDecisions(String aDate, Model model) throws IOException {
		...
		List<Decision> descionsList = handleRequests.searchDecisions(aDate, model);
	...
	/*
	 * Β.1.3 --> ΩΘΓΝ9-Ψ7Ψ
	 * Β.2.1 --> ΩΚΣΛ469Β7Τ-69Η
	 * Β.2.2 --> 9ΠΔ5Ι-ΕΑ1
	 * Δ.1   --> 6ΣΝΤΟΞΘΟ-165
	 * Δ.2.1 --> 7ΖΒΘΟΞΧΓ-Ι9Ρ
	 * Δ.2.2 --> Ω3ΙΔΟΡΛΟ-ΦΤΕ
	 * corrected --> ΒΝΩ9Ι-Μ10
	 * no AFF provided --> ΒΙ6ΝΟΡΥΑ-8ΦΦ
	 * noVatOrg as 9... --> 4ΑΘΓΩ0Ο-Ι
